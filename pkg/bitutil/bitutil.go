...
var (
	lsbBitMask        = [8]byte{1, 2, 4, 8, 16, 32, 64, 128}
	lsbFlippedBitMask = [8]byte{254, 253, 251, 247, 239, 223, 191, 127}
	msbBitMask        = [8]byte{128, 64, 32, 16, 8, 4, 2, 1}
	msbFlippedBitMask = [8]byte{127, 191, 223, 239, 247, 251, 253, 254}
)
...
// BitIsSet returns true if the bit at index i in buf is set (1) using least-significant bit.
func BitIsSet(buf []byte, i int) bool { return (buf[uint(i)/8] & lsbBitMask[byte(i)%8]) != 0 }

// BitIsNotSet returns true if the bit at index i in buf is not set (0).
func BitIsNotSet(buf []byte, i int) bool { return (buf[uint(i)/8] & lsbBitMask[byte(i)%8]) == 0 }

// SetBit sets the bit at index i in buf to 1.
func SetBit(buf []byte, i int) { buf[uint(i)/8] |= lsbBitMask[byte(i)%8] }

// ClearBit sets the bit at index i in buf to 0.
func ClearBit(buf []byte, i int) { buf[uint(i)/8] &= lsbFlippedBitMask[byte(i)%8] }
...
// BitIsSetMSB returns true if the bit at index i in buf is set (1) using least significant bit.
func BitIsSetMSB(buf []byte, i int) bool { return (buf[uint(i)/8] & msbBitMask[byte(i)&7]) != 0 }
